...
exports.crearOrden = async (req, res) => {
  ...
  try {
    ...
    if (!items || items.length === 0) {
      await t.rollback();
      return res.status(400).json({ success: false, mensajes: ['items debe contener al menos 1 producto'] });
    }

    if (!direccion || !direccion.calle || !direccion.ciudad || !direccion.codigoPostal || !direccion.pais) {
      await t.rollback();
      return res.status(400).json({ success: false, mensajes: ['Todos los campos de dirección son requeridos'] });
    }
    ...
    for (const item of items) {
      const producto = await Product.findByPk(item.productoId, { transaction: t });
